...
def test_settings_handler_structure():
    ...
    assert (
        "class SettingsHandler(object):" in settings_content
    ), "SettingsHandler class not found"
    print("✓ SettingsHandler base class exists")

    # Check for AlexaSettingsHandler implementation
    assert (
        "class AlexaSettingsHandler(SettingsHandler):" in settings_content
    ), "AlexaSettingsHandler class not found"
    ...
    assert (
        "def set_location(self, location" in settings_content
    ...
    assert (
        "def set_metrics(self, metrics" in settings_content
    ...
    assert (
        "self.attr_mgr = handler_input.attributes_manager" in settings_content
    ), "attributes_manager not used"
    assert (
        "persistent_attrs = self.attr_mgr.persistent_attributes" in settings_content
    ), "persistent_attributes not used"
    assert (
        "self.attr_mgr.save_persistent_attributes()" in settings_content
    ), "save_persistent_attributes not called"
    ...
    assert (
        "self.settings_handler = settings_handler" in content
    ), "settings_handler not stored in Skill"
    assert (
        "self.settings_handler.get_location()" in content
    ), "settings_handler.get_location() not used"
    assert (
        "self.settings_handler.set_location(" in content
    ), "settings_handler.set_location() not used"
    assert (
        "self.settings_handler.get_rate()" in content
    ), "settings_handler.get_rate() not used"
    assert (
        "self.settings_handler.set_rate(" in content
    ), "settings_handler.set_rate() not used"
    assert (
        "self.settings_handler.get_pitch()" in content
    ), "settings_handler.get_pitch() not used"
    assert (
        "self.settings_handler.set_pitch(" in content
    ), "settings_handler.set_pitch() not used"
    assert (
        "self.settings_handler.get_metrics()" in content
    ), "settings_handler.get_metrics() not used"
    assert (
        "self.settings_handler.set_metrics(" in content
    ), "settings_handler.set_metrics() not used"
    print("✓ Skill uses settings_handler methods")

    # Check that BaseIntentHandler creates settings_handler
    assert (
        "settings_handler = AlexaSettingsHandler(handler_input)" in content
    ), "BaseIntentHandler doesn't create settings_handler"
    assert (
        "Skill(handler_input, get_cache_handler(), settings_handler)" in content
        or "skill = Skill(handler_input," in content
    ...
    assert (
        "_load_user_settings" not in content
        or "def _load_user_settings(self):" not in content
    ), "Old _load_user_settings method still exists in Skill"
    assert (
        "_save_user_settings" not in content
        or "def _save_user_settings(self):" not in content
...
def test_settings_separation():
    ...
    assert (
        "attr_mgr.persistent_attributes" not in skill_class_content
    ), "Skill class still directly accesses persistent_attributes"
    assert (
        "save_persistent_attributes()" not in skill_class_content
    ...
    assert (
        "self.settings_handler.get_location()" in skill_class_content
    ), "user_location property doesn't delegate to settings_handler"
    assert (
        "self.settings_handler.set_location(" in skill_class_content
