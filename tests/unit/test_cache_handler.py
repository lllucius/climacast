...
def test_cache_handler_structure():
    ...
    assert (
        "from storage.cache_handler import CacheHandler" in lambda_content
    ...
    assert (
        "class CacheHandler(object):" in content or "class CacheHandler:" in content
    ), "CacheHandler class not found"
    ...
    assert (
        "def get(self, cache_type, cache_id):" in content
        or "def get(self, cache_type: str, cache_id: str)" in content
    ), "get method not found"
    assert (
        "def put(self, cache_type, cache_id, cache_data" in content
        or "def put(self, cache_type: str, cache_id: str, cache_data" in content
    ), "put method not found"
    assert (
        "def get_location(self, location_id):" in content
        or "def get_location(self, location_id: str)" in content
    ), "get_location method not found"
    assert (
        "def put_location(self, location_id, location_data" in content
        or "def put_location(self, location_id: str, location_data" in content
    ), "put_location method not found"
    assert (
        "def get_station(self, station_id):" in content
        or "def get_station(self, station_id: str)" in content
    ), "get_station method not found"
    assert (
        "def put_station(self, station_id, station_data" in content
        or "def put_station(self, station_id: str, station_data" in content
    ), "put_station method not found"
    assert (
        "def get_zone(self, zone_id):" in content
        or "def get_zone(self, zone_id: str)" in content
    ), "get_zone method not found"
    assert (
        "def put_zone(self, zone_id, zone_data" in content
        or "def put_zone(self, zone_id: str, zone_data" in content
    ), "put_zone method not found"
    print("✓ All cache methods defined")

    # Check lambda_function.py for cache handler factory function
    assert (
        "def get_cache_handler()" in lambda_content
    ), "get_cache_handler factory function not found"
    print("✓ Cache handler factory function exists")

    # Check that User class is removed
    assert (
        "class User(Base):" not in lambda_content
    ), "User class still exists (should be removed)"
    ...
    assert (
        ...
        in lambda_content
    ), "Skill doesn't accept cache_handler"
    ...
    assert (
        "class AlexaSettingsHandler" in settings_content
    ), "AlexaSettingsHandler not found"
    ...
    assert (
        "def __init__(self, event, cache_handler=None):" in base_content
        or "def __init__(self, event: Dict[str, Any], cache_handler=None)"
        in base_content
    ...
    assert (
        "from ask_sdk_dynamodb.adapter import DynamoDbAdapter" in lambda_content
    ), "DynamoDbAdapter import not found"
    assert (
        "persistence_adapter = DynamoDbAdapter" in lambda_content
    ), "DynamoDbAdapter not instantiated"
...
def test_cache_key_structure():
    ...
    assert (
        '"pk": f"{cache_type}{cache_id}"' in content
        or "'pk': f\"{cache_type}{cache_id}\"" in content
    ), "PK format incorrect"
    assert (
        '"sk": "data"' in content or "'sk': \"data\"" in content
    ), "SK format incorrect"
...
def test_single_table_design():
    ...
    assert (
        'DDB.Table("LocationCache")' not in content
    ), "Old LocationCache table reference found"
    assert (
        'DDB.Table("StationCache")' not in content
    ), "Old StationCache table reference found"
    assert (
        'DDB.Table("UserCache")' not in content
    ), "Old UserCache table reference found"
    assert (
        'DDB.Table("ZoneCache")' not in content
    ), "Old ZoneCache table reference found"
    print("✓ Old separate table references removed")

    # Check that single table is used
    assert (
        "self.table = self.ddb.Table(table_name)" in content
        or "self.table = DDB.Table(table_name)" in content
    ), "Single table not used in CacheHandler"
    print("✓ Single table design implemented")

    # Check that cache_data is used
    assert (
        '"cache_data": cache_data' in content or "'cache_data': cache_data" in content
    ), "cache_data not stored"
    assert (
        'item.get("cache_data"' in content or "item.get('cache_data'" in content
    ), "cache_data not retrieved"
