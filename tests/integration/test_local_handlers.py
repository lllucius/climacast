...
def test_local_cache_handler():
    ...
    assert (
        "from storage.local_handlers import LocalJsonCacheHandler" in lambda_content
    ...
    assert (
        "class LocalJsonCacheHandler(object):" in content
        or "class LocalJsonCacheHandler:" in content
...
def test_local_settings_handler():
    ...
    assert (
        "from storage.local_handlers import" in lambda_content
        and "LocalJsonSettingsHandler" in lambda_content
    ...
    assert (
        "class LocalJsonSettingsHandler(SettingsHandler):" in content
        or "class LocalJsonSettingsHandler:" in content
    ...
    assert (
        "def get_location(self):" in content or "def get_location(self) ->" in content
    ), "get_location method not found"
    assert (
        "def set_location(self, location):" in content
        or "def set_location(self, location: str)" in content
    ), "set_location method not found"
    assert (
        "def get_rate(self):" in content or "def get_rate(self) ->" in content
    ), "get_rate method not found"
    assert (
        "def set_rate(self, rate):" in content
        or "def set_rate(self, rate: int)" in content
    ), "set_rate method not found"
    assert (
        "def get_pitch(self):" in content or "def get_pitch(self) ->" in content
    ), "get_pitch method not found"
    assert (
        "def set_pitch(self, pitch):" in content
        or "def set_pitch(self, pitch: int)" in content
    ), "set_pitch method not found"
    assert (
        "def get_metrics(self):" in content or "def get_metrics(self) ->" in content
    ), "get_metrics method not found"
    assert (
        "def set_metrics(self, metrics):" in content
        or "def set_metrics(self, metrics: list)" in content
    ), "set_metrics method not found"
    print("✓ LocalJsonSettingsHandler has all required methods")

    # Verify it uses local file storage
    assert (
        "os.makedirs" in content
    ), "LocalJsonSettingsHandler doesn't create directories"
...
def test_test_mode_integration():
    ...
    assert (
        "os.environ.get('CLIMACAST_TEST_MODE'" in content
    ), "CLIMACAST_TEST_MODE environment check not found"
    print("✓ Test mode uses environment variable")

    # Check that get_skill_helper uses environment variable
    assert (
        "is_test_mode = os.environ.get('CLIMACAST_TEST_MODE'" in content
    ...
    assert (
        "os.environ['CLIMACAST_TEST_MODE'] = 'true'" in content
