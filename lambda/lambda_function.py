...
from weather_processor import (
    ...
    METRICS, DAYS, MONTH_NAMES, MONTH_DAYS, MONTH_DAYS_XLATE, QUARTERS, SETTINGS,
    notify
)
from cache_adapter import DynamoDBCacheAdapter

"""
    Lambda function configuration and initialization
"""
VERSION = 2
REVISION = 0
...
# Get geocoding API key - prefer HERE.com, fall back to MapQuest
HERE_API_KEY = os.environ.get("here_api_key", "")
MQID = os.environ.get("mapquest_id", "")

# Determine which geocoding service to use
if HERE_API_KEY:
    GEOCODER_API_KEY = HERE_API_KEY
    GEOCODER_TYPE = "here"
    print("Using HERE.com geocoding API")
elif MQID:
    GEOCODER_API_KEY = MQID
    GEOCODER_TYPE = "mapquest"
    print("Using MapQuest geocoding API (deprecated - please switch to HERE.com)")
else:
    GEOCODER_API_KEY = None
    GEOCODER_TYPE = None
    print("WARNING: No geocoding API key configured")
...
class BaseIntentHandler(AbstractRequestHandler):
    ...
    def get_user_and_location(self, handler_input):
        ...
        # Create cache adapter for DynamoDB
        cache_adapter = DynamoDBCacheAdapter(
            attributes_manager=handler_input.attributes_manager,
            persistence_table_name=PERSISTENCE_TABLE_NAME,
            persistence_region=PERSISTENCE_REGION
        )

        # Load user profile
        user = User(event, user_id, cache_adapter)

        # Try to load default location
        loc = None
        if user.location:
            location_obj = Location(event, geocoder_api_key=GEOCODER_API_KEY, 
                                   geocoder_type=GEOCODER_TYPE, cache_adapter=cache_adapter)
    ...
    def get_alerts(self, handler_input, event, loc):
        ...
        alerts = Alerts(event, loc.countyZoneId, cache_adapter)
        if len(alerts) == 0:
    ...
    def get_current(self, handler_input, event, user, loc, metrics):
        ...
        obs = Observations(event, loc.observationStations, cache_adapter=cache_adapter)
        if obs.is_good:
    ...
    def get_forecast(self, handler_input, event, user, loc, stime, etime, sname, metrics):
        ...
        fulltext = ""
        gp = GridPoints(event, loc.tz, loc.cwa, loc.grid_point, cache_adapter)
